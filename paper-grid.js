...
class PaperGrid extends mixinBehaviors([GestureEventListeners], PolymerElement) {
        ...
        if (isPlaceholder && !removed) {
            ...
            Array.from(resizers).forEach(resizer => {
                if (addOrRemoveResizeListener && !resizer._hasResizeListener) {
                    ...
                    this._removeEventListenerFromNode(resizer, 'track', resizeHandler);
                    resizer._hasResizeListener = false;
                }
            });
            if (addOrRemoveMoveListener && !node._hasMoveListener) {
                this._addEventListenerToNode(node, 'track', moveHandler);
                node._hasMoveListener = true;
            } else if (!addOrRemoveMoveListener) {
                this._removeEventListenerFromNode(node, 'track', moveHandler);
    ...
    serialize() {
        return Array.from(this.children)
            .filter((element) => !element.hasAttribute('placeholder'))
            .map(child => {
                return {
...
window.customElements.define('paper-grid', PaperGrid);
